...
from .train import load_policies

OPPONENT_TYPES = {"simple": SimpleBot, "random": RandomBot}


def build_players(
    save_dir: str | Path, num_players: int, opponent: str, rng: random.Random
) -> list[Player]:
    bidding_policy, card_policy = load_policies(save_dir)
    opponent_cls = OPPONENT_TYPES[opponent]

    players: list[Player] = [
        RLPlayer("RLBot", bidding_policy, card_policy, training=False, deterministic=True, rng=rng)
    ]
...
def evaluate(
    ...
    for _ in range(num_games):
        players = build_players(save_dir, num_players, opponent, rng)
